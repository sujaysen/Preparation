...
for row in csvreader:
	try:
		...
		for i in range(4):
			...
			doc = {"_id":idd,"date":date,"action":"buy","buy":buy,"stoploss":stoploss,"margin":margin,"income_status":status,"high":high,"low":low,"prev_close":prev_close,"today_close":today_close,"volume":volume,"opening status":open_status}
		...
		for i in range(4):
			...
			doc = {"_id":idd,"date":date,"action":"sell","sell":sell,"stoploss":stoploss,"margin":margin,"income_status":status,"high":high,"low":low,"prev_close":prev_close,"today_close":today_close,"volume":volume,"opening status":open_status}
			mycol.update_one({"_id": idd},{"$set": doc},upsert=True)
			#mycol.insert_one(doc)
			margin = margin + 0.5
